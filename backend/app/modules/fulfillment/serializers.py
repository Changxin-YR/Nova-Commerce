...
def _item_out(row: Any, sku_by_line: Mapping[int, int]) -> FulfillmentItemOut:
    ...
    ## ``sku_id`` is resolved through the order line

    ``fulfillment_items`` stores ``order_item_id`` and the two name snapshots, and
    deliberately does **not** duplicate ``sku_id`` (see ``fulfillment/models.py``:
    a package line whose ``sku_id`` disagreed with the order line it claims to be
    part of would be an undetectable shipping error). The frozen wire shape does carry
    ``sku_id`` (``API_CONTRACT`` section 5, ``FulfillmentItemOut``), so it is resolved
    here through the order line, which is the single source of truth for it.

    ``sku_by_line`` is passed in rather than looked up per row because a list
    endpoint would otherwise run one query per line - the N+1 that only becomes
    visible on an order that shipped in five packages with four lines each. The
    caller loads one mapping per page.

    ``row.sku_id`` is preferred when the model provides it (a model-level property
    that resolves the same thing), so this stays correct whichever way the mapping
    is supplied, and a row that already knows its SKU costs no mapping lookup at all.
    """
    direct = getattr(row, "sku_id", None)
    sku_id = direct if direct is not None else sku_by_line[row.order_item_id]
