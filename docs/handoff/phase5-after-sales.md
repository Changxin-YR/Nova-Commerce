...
3. **The allocation basis.** Weight each line by its **remaining** capacity (`payable_amount - refunded_amount`), not its total payable: `sum of shares == amount, and share_i <= line_i.remaining`. The design's literal `amount * payable / approved_amount` over-commits a line when `approved_amount <  claimed payables`; the captain is correcting section 6.2.
...
(`RefundOutcome.refund`, `.replayed`, `.split`). It quotes **no prices** and commits **once**.
